...
void handleScreenS(const std::string& name, Scheduler* scheduler, std::map<std::string, ScreenSession>& screens) {
    ...
    if (!scheduler->isRunning()) {
        ...
        std::thread soloWorker([scheduler, &screens, name, assignedCore]() {
            ...
            while (processActive) {
                // Add safety check for scheduler validity
                if (!scheduler) {
                    break;
                }

                {
                    ...
                    for (auto& proc : procList) {
                        if (proc.name == name && !proc.finished &&
                            ...
                            if (proc.executedCommands >= proc.totalCommands) {
                                ...
                                soloProcessCount--;
                                processActive = false; // Exit the loop
                            }
                    ...
                    // If process not found or no more work, exit
                    if (!processFound) {
                        coreToProcess[assignedCore] = "";
                        soloProcessCount--;
                        processActive = false;
        ...
        soloWorker.detach(); // Keep detached but ensure it exits properly
    }
...
std::vector<Instruction> generateRandomInstructions(const std::string& procName, int count, int nestLevel) {
    ...
    // Always initialize "x" variable with value 0
    if (nestLevel == 0) { // Only for top-level processes
        Instruction xInit;
    ...
    for (int i = 0; i < count; ++i) {
        ...
        switch (t) {
            case 0: { // PRINT - Modified to print "x" value
                instr.type = ICommand::InstrType::PRINT;
                instr.msg = "Value from: " + procName;
                instr.printVar = "x"; // Print the x variable
                break;
            }
            ...
            case 2: { // ADD - Modified to increment x
                instr.type = ICommand::InstrType::ADD;
                instr.var1 = "x"; // Always modify x
                instr.var2 = "x"; // x = x + 1
                instr.var3 = "1";
                break;
            }
...
int main() {
    ...
    while (menuState) {
        ...
        if (inputCommand.find("initialize") != std::string::npos) {
            ...
            // Wait for solo threads to finish
            ...
            // Final delay to ensure all threads are done
